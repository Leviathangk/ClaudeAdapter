...
use axum::{Router, body::{Body, Bytes, to_bytes}, extract::Json, http::{Request, Response, StatusCode}, routing::post};
    ...
    let upstream_app = Router::new().route(
        ...
        post(|Json(_payload): Json<Value>| async move {
            let stream = futures_util::stream::iter(vec![
                Ok::<Bytes, Infallible>(Bytes::from("data: {\"choices\":[{\"delta\":{\"content\":\"hel\"}}]}\n\n")),
                Ok::<Bytes, Infallible>(Bytes::from("data: {\"choices\":[{\"delta\":{\"content\":\"lo\"}}]}\n\n")),
                Ok::<Bytes, Infallible>(Bytes::from("data: {\"choices\":[{\"finish_reason\":\"stop\"}]}\n\n")),
